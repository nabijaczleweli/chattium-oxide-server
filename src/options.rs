	...
	pub fn parse() -> Options {
			...
			if yaml.add_files(vec![config]) {
				if let Some(yaml) = yaml.read_all_files().as_ref().map(|all| {
					...
					if let Some(map) = yaml["ssl"].as_hash() {
						ssl_cert = if let Some(cert) = map.get(&Yaml::String("cert".to_string())) {cert.as_str().map(|c| c.to_string())} else {None};
						ssl_key  = if let Some(key)  = map.get(&Yaml::String("key".to_string()))  {key .as_str().map(|k| k.to_string())} else {None};
					}
		...
		Options{
			...
			ssl: match (ssl_cert, ssl_key) {
				...
				(None, Some(key)) => {
					println!("SSL key specified (\"{}\"), but certificate not specified", key);
					None
				},
				(Some(cert), None) => {
					println!("SSL key not specified, but certificate specified (\"{}\")", cert);
					None
				},
				(None, None) => None,
			},
